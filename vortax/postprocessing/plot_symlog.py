...
def get_symlog_bins(
    ...
    if min_val < -linear_threshold:
        neg_bins = -np.logspace(
            np.log10(max(abs(min_val), linear_threshold)),
    ...
    if max_val > linear_threshold:
        pos_bins = np.logspace(
            np.log10(linear_threshold),
            np.log10(max(max_val, linear_threshold)),
            n_bins // 3,
        )
    else:
        pos_bins = []
...
def plot_symlog_distribution(
    ...
    if ax is None:
        ax = plt.gca()
    plt.sca(ax)
    bins = get_symlog_bins(
        np.min(data), np.max(data), linear_threshold=linear_threshold
    )
    ...
    if add_reference_lines:
        ...
        ax.axvline(
            x=median_val, color="g", linestyle="--", label=f"Median: {median_val:.3e}"
        )
        ...
        for p in percentiles:
            ax.axvline(
                ...
                color="orange",
                linestyle=":",
                label=f"{p}% percentile: {np.percentile(data, p):.3e}",
            )
