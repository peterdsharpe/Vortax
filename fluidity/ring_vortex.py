...
def get_induced_velocity_ring_vortex(
    ...
    """
    Get the velocity induced by a ring vortex at a given point.
    """
...
def get_induced_velocity_mesh_ring_vortices(
    ...
    """
    Get the velocity induced by the mesh at a given point.

    Assumes that each face of the mesh has a ring vortex. By computing the induced velocity from the whole mesh at once,
    as opposed to vmapping over individual ring vortices, we can save some repeated calculations.

    Args:
        mesh: The mesh to get the velocity from
        query_point: The field point where the velocity is evaluated.
        vortex_strengths: The strengths of the ring vortices. If not provided, unit strengths are assumed.
        radius: The radius of the ring vortices, used to prevent singularities.
        dotted_with: The vector to dot the velocity with. Also determines the shape of the returned array.

    Returns:
        The velocity induced by the mesh at the given point
    """
    # Set defaults
