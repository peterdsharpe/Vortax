...
@eqx.filter_jit
def lhs(source_strengths: Float[Array, " n_faces"]) -> Float[Array, " n_faces"]:
    def get_induced_velocity_single_pair(source_i: int, target_i: int) -> float:
        return get_induced_velocity_source(
            source_point=face_centers[source_i],
            query_point=face_centers[target_i],
            source_strength=source_strengths[source_i] * face_areas[source_i],
            radius=1e-4 * smallest_face_scale,
    ...
    return jnp.concatenate(
        [
            ...
            # jnp.sum(source_strengths, keepdims=True)
        ]
    )
...
print(f"Velocity MAE error: {np.mean(np.abs(lhs(source_strengths) - rhs))}")
print(f"Source strengths mean: {np.mean(source_strengths)}")
...
def compute_velocity(query_point: vec3) -> vec3:
    """
    Compute the velocity at a query point due to all sources on the mesh plus freestream.
...
### Create a new plotter to visualize the mesh with source strengths
# intensity = np.maximum(0, np.log10(np.abs(source_strengths) + 1e-10))
...
pl_mesh.add_mesh(
    mesh.to_pyvista(),
    scalars=source_strengths,
    label="Source strengths",
    cmap="RdBu",
    clim=np.array([-1, 1]) * np.max(np.abs(np.percentile(source_strengths, (5, 95)))),
    # opacity=opacities,
)
pl_mesh.add_axes()
pl_mesh.show_bounds()
...
# Visualize the distribution of source strengths

plot_symlog_distribution(source_strengths, show=True)
